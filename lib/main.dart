    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          switch (user['role']) {
            case 'student':
              nextScreen = StudentMapScreen(
                assignedBusId: 'bus1', // You can get this from backend later
                userName: '${user['firstName']} ${user['lastName']}',
                studentId: user['studentId'],
              );
              break;
            case 'driver':
              nextScreen = DriverMapScreen(
                userName: '${user['firstName']} ${user['lastName']}',
    ...
    if (_formKey.currentState!.validate() && _agreeToTerms) {
      try {
        ...
        if (response.statusCode == 201) {
          ...
          // Navigate to login screen after brief delay
                      ...
                      children: [
                        ...
                        TextFormField(
                          ...
                          validator: (value) {
                            ...
                            // Use the same email validation as student registration
    ...
    if (_formKey.currentState!.validate() && _agreeToTerms) {
      try {
        ...
        if (response.statusCode == 201) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('✅ Driver account created successfully! Please login with your credentials.'),
              backgroundColor: Colors.green,
              duration: Duration(seconds: 3),
            ),
          );
            ...
            DrawerHeader(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                  Text(
                    widget.userName ?? 'Student Profile',
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                  ...
                  Text(
                    'ID: ${widget.studentId ?? 'N/A'}',
                    style: const TextStyle(color: Colors.white70, fontSize: 14),
                  ),
                  Text(
                    'Assigned Bus: ${widget.assignedBusId.toUpperCase()}',
...
class _DriverMapScreenState extends State<DriverMapScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Welcome ${widget.userName ?? 'Driver'}'),
        backgroundColor: Colors.blue.shade900,
        elevation: 0,
        actions: [
            ...
            DrawerHeader(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                  Text(
                    widget.userName ?? 'Driver Profile',
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                  ...
                  Text(
                    'License: ${widget.licenseNumber ?? 'N/A'}',
                    style: const TextStyle(
                      color: Colors.white70,
                      fontSize: 14,
                    ),
                  ...
                  Text(
                    'Vehicle: ${widget.vehicleNumber ?? 'N/A'}',
                    style: const TextStyle(
                      color: Colors.white70,
                      fontSize: 12,
                    ),
...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Welcome ${widget.userName ?? 'Admin'}"),
        backgroundColor: Colors.blue.shade900,
        elevation: 0,
        actions: [
          IconButton(
...
class _ManageStudentsScreenState extends State<ManageStudentsScreen> {
  ...
  Future<void> _fetchStudents() async {
    try {
      ...
      if (response.statusCode == 200) {
        final allUsers = jsonDecode(response.body) as List<dynamic>;
        ...
        setState(() {
          students = allUsers
              ...
              .map((user) => {
                    ...
                    'assignedBus': 'bus1', // Default for now, you can get this from backend
                    'busName': 'Bus 1', // Default for now, you can get this from backend
                    'profilePic': null,
                  })
              .toList();
          isLoading = false;
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          drivers = allUsers
              ...
              .map((user) => {
                    'id': 'DRV${user['id'].toString().padLeft(3, '0')}', // Generate driver ID
                    'name': '${user['firstName']} ${user['lastName']}',
                    'email': user['email'] ?? '',
                    'phone': user['phoneNumber'] ?? '',
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      _buildDriverDetailRow(Icons.directions_bus, 'Vehicle', driver['vehicle']),
                      const SizedBox(height: 8),
